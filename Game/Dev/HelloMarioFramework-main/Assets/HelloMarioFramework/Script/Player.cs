...
namespace HelloMarioFramework
{
    public class Player : MonoBehaviour
    {
        ...
        //Speed multiplier
        public float speedMultiplier = 1f; //0.5-1
        public float jumpMultiplier = 1f; //0.8-1
        ...
        void FixedUpdate()
        {
            ...
            if (controlsEnabled)
            {
                ...
                if (speedCap && mvmntSpeed.sqrMagnitude > 81f * speedMultiplier)
                {
                    mvmntSpeed.Normalize();
                    mvmntSpeed = mvmntSpeed * 9f * speedMultiplier;
                }
                else if (!speedCap)
                {
                    Vector2 anotherCheck = new Vector2(mvmntSpeed.x, mvmntSpeed.y);
                    mvmntSpeed.Normalize();
                    mvmntSpeed = mvmntSpeed * 18f * speedMultiplier;
